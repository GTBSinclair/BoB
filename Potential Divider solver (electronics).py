...
# --- Resistors currently available in the BoB electronics boxes
R = [150, 500, 9000, 3000, 240, 100, 5600, 10000, 270, 1000, 820]
...
inputX = int(input("\nType 1 to add more resistors, otherwise type 0 to use available BoB resistors only: "))
if inputX == 1:
    inputY = int(input("How many resistors need to be added? : "))
    print('\nUse the SI unit Ω:\n')
...
for i in range(len(R1)):
    for j in range(len(R2)):
        if Voltages[0] == (R2[j]*Voltages[1])/(R1[i]+R2[j]): # Exact calculation
            P.append([[R2[j],R1[i]],(R2[j]*Voltages[1])/(R1[i]+R2[j]), 
                      Voltages[1]/(R1[i]+R2[j])]) # Current (A) is calculated at the end of P.append
        elif ((R2[j]*Voltages[1])/(R1[i]+R2[j]))>Voltages[0]-Voltages[2] and ((R2[j]*Voltages[1])/(R1[i]+R2[j]))<Voltages[0]+Voltages[3]: # Approximate calculation
            P2.append([[R2[j],R1[i]],(R2[j]*Voltages[1])/(R1[i]+R2[j]),
                       Voltages[1]/(R1[i]+R2[j])]) # Current (A) is calculated at the end of P2.append
...
if P2 != []:
    ...
    for l in range(len(P2)):
        print('[R2,R1] = ' + str(Divide_by_1E3(P2[l][0])) 
              + ' kΩ\n' + 'Vout = ' + str(round(P2[l][1],2)) 
              + ' V\n' + 'Iout = ' + str(round(Times_by_1E3(P2[l][2]),2)) + ' mA\n')
...
if P != []:
    ...
    for u in range(len(P)):
        print('[R2,R1] = ' + str(Divide_by_1E3(P[u][0])) + ' kΩ\n' + 'Vout = ' + 
              str(round(P[u][1],2)) + ' V\n' + 'Iout = ' 
              + str(round(Times_by_1E3(P[u][2]),2)) + ' mA\n')
